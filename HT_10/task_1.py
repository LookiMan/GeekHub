...
def post_menu(function: Callable) -> Callable:
    def menu_wrapper() -> None:
        ...
        command = safe_input("[<] введіть число", validator=lambda v: int(v) in (1, 2))
...
def balance_replenishment_screen() -> None:
    ...
    contribution = safe_input("[<] сума внесення", validator=int, post_processor=int)
...
def balance_withdraw_screen() -> None:
    user_id = get_user_id_form_session()
    balance = utils.load_user_balance(user_id)
    withdraw = safe_input("[<] сума для зняття", validator=int, post_processor=int)
...
def banknote_replenishment_screen() -> None:
    ...
    utils.increase_availble_banknotes({nominal: amount}, avalible_banknotes)
    utils.save_banknotes(avalible_banknotes)


@post_menu
def currency_courses():
...
def collection_screen() -> None:
    ...
    command = safe_input("[<] введіть число", validator=lambda v: int(v) in range(1, 5))
...
def user_screen() -> None:
    ...
    command = safe_input("[<] введіть число", validator=lambda v: int(v) in range(1, 6))
...
def service_screen() -> None:
    ...
    command = safe_input("[<] введіть число", validator=lambda v: int(v) in range(1, 7))
